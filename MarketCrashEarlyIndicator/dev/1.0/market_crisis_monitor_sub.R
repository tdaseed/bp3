...
f_out <- "C:/HyperParameter_FineTune_Eq/"
f_out <- "C:/HyperParameter_FineTune_FX/"
f_out <- "C:/HyperParameter_FineTune_Oil/"
...
for (md in seq(3,7,1)){
  for (nrounds in seq(5,20,5)){
    # print(paste("max_depth=", toString(md)), "nrounds=", toString(nrounds))
    backtester(df=df, tgt_ind=tgt_ind, w=26, s=1, d=2, n=20, TH=TH, nrounds=nrounds, Use_TDA=TRUE, max_dep=md, f_out=f_out)
  }
}
...
for (md in seq(3,7,1)){
  for (nrounds in seq(5,20,5)){
    # print(paste("max_depth=", toString(md)), "nrounds=", toString(nrounds))
    backtester(df=df, tgt_ind=tgt_ind, w=26, s=1, d=2, n=20, TH=TH, nrounds=nrounds, Use_TDA=FALSE, max_depth=md, f_out=f_out)
  }
}
...
f_out <- "C:/bp3/MarketCrashEarlyIndicator/dev/1.0/results/sub/"
...
feaBuilder <- function(df_t){
  ...
  fea <- data.frame(ret_1w_tgt, # ret_1w_tgt, ret_1w_DXY,ret_1w_Gld,ret_1w_Oil,
                    ret_2q_tgt, # ret_2q_tgt, ret_2q_DXY,ret_2q_Oil,#ret_2q_Gld,
                    # sprd_2y10y,sprd_2y5y,
                    # ret_1w_FF_Rm_Rf, ret_1w_FF_SMB, ret_1w_FF_HML,
                    vol_tgt) # vol_tgt,vol_DXY,vol_Oil) #vol_Gld,
...
feaBuilder_TDA <- function(df_t){
  ...
  fea <- data.frame(ret_1w_tgt, # ret_1w_tgt, ret_1w_DXY,ret_1w_Gld,ret_1w_Oil,
                    ...
                    vol_tgt, # vol_tgt, vol_DXY,vol_Oil,#vol_Gld,
                    # ret_1w_FF_Rm_Rf, ret_1w_FF_SMB, ret_1w_FF_HML,
                    tda_tgt) # tda_tgt,tda_DXY,tda_Oil)#tda_Gld)
  return(fea)
}
